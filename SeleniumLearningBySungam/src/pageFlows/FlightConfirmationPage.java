...
public class FlightConfirmationPage {
	...
	public void validation_Confirm(WebDriver driver) {
		 ...
		 * String actual_Departing_City =cities[0] ; String actual_Arriving_City =
		 * cities[1];
		...
		String actual_DepartingInfo_Date = actual_DepartingInfo.substring(
				actual_DepartingInfo.indexOf(arriveCity) + arriveCity.length(), actual_DepartingInfo.indexOf(" @ "));

		// First class
		int departBeginIndex_class = actual_DepartingInfo.indexOf('F');
		System.out.println("departBeginIndex_class=" + departBeginIndex_class);
		int departEndIndex_class = actual_DepartingInfo.indexOf('$');
		System.out.println("departEndIndex_class=" + departEndIndex_class);
		...
		// Depart price
		int departBeginIndex_price = actual_DepartingInfo.indexOf('$') + 1;
		System.out.println("departBeginIndex_price=" + departBeginIndex_price);
		int departEndIndex_price = actual_DepartingInfo.lastIndexOf(' ');
		System.out.println("departEndIndex_price=" + departEndIndex_price);
		...
		// First class
		int returnBeginIndex_class = actual_ReturningInfo.indexOf('F');
		System.out.println("returnBeginIndex_class=" + returnBeginIndex_class);
		int returnEndIndex_class = actual_ReturningInfo.indexOf('$');
		System.out.println("returnEndIndex_class=" + returnEndIndex_class);
		...
		// return price
		int returnBeginIndex_price = actual_ReturningInfo.indexOf('$') + 1;
		System.out.println("returnBeginIndex_price=" + returnBeginIndex_price);
		int returnEndIndex_price = actual_ReturningInfo.lastIndexOf(' ');
		System.out.println("returnEndIndex_price=" + returnEndIndex_price);
		...
		/*
		 * if(expected_ConfirmationText.equals(actual_ConfirmationText)) System.out.
		 * println("Pass - expected_ConfirmationText = actual_ConfirmationText"); else
		 * System.out.
		 * println("Pass - expected_ConfirmationText != actual_ConfirmationText");
		 */
		...
		ReportingUtils.reportResult("Done", "Flight Confirmation",
				"Flight Confirmation Text Validation" + actual_ConfirmationText);

		// Validation - Depart City
		String verdict_DepartCity = ReportingUtils.compare(expected_Departing_City, actual_Departing_City);
		ReportingUtils.reportResult("Done", "Flight Confirmation - Departing City",
				"Flight confirmation for departing city" + actual_Departing_City);

		// Validation - Return City
		String verdict_ReturnCity = ReportingUtils.compare(expected_Arriving_City, actual_returning_City);
		ReportingUtils.reportResult("Done", "Flight Confirmation - Returning City",
				"Flight confirmation for returning city" + actual_returning_City);

		// Validation - Class
		ReportingUtils.reportResult("Done", "Flight confirmation - Class",
				"Flight confirmation Class - " + actual_Depart_Class);
